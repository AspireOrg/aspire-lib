...
# Filled orders may not be re‐opened, so only orders not involving GASP (and so
...
def exact_penalty(db, address, block_index, order_match_id):
    # Penalize addresses that don’t make GASP payments. If an address lets an
    # order match expire, expire sell GASP orders from that address.
    cursor = db.cursor()
...
def cancel_order(db, order, status, block_index):
    ...
    if order['give_asset'] != config.BTC:    # Can’t credit GASP.
...
def cancel_order_match(db, order_match, status, block_index):
    ...
    orders = list(cursor.execute('''SELECT * FROM orders WHERE tx_index = ?''', (order_match['tx1_index'],)))
    assert len(orders) == 1
    tx1_order = orders[0]
...
def parse(db, tx, message):
    ...
    if status == 'open':
        if give_asset != config.BTC:  # No need (or way) to debit GASP.
...
def match(db, tx, block_index=None):
    ...
    for tx0 in order_matches:
        ...
        # Make sure that that both orders still have funds remaining (if order involves GASP, and so cannot be ‘filled’).
        ...
        if tx0_price > tx1_inverse_price:
            ...
            min_btc_quantity = 0.0001 * config.UNIT  # 0.0001 GASP
            ...
            fee = 0
            # Deduct fee_required from provided_remaining, etc., if possible (else don’t match).
            ...
            if config.BTC in (tx1['give_asset'], tx1['get_asset']):
                ...
                status = 'completed'
                # Credit.
                util.credit(db, tx1['source'], tx1['get_asset'], forward_quantity, action='order match', event=order_match_id)
                util.credit(db, tx0['source'], tx0['get_asset'], backward_quantity, action='order match', event=order_match_id)

            # Debit the order, even if it involves giving aspiregas, and so one
            ...
            # tx0
            tx0_status = 'open'
            if tx0_give_remaining <= 0 or tx0_get_remaining <= 0:
...
def expire(db, block_index):
    ...
    # Expire orders and give refunds for the quantity give_remaining (if non-zero; if not GASP).
    cursor.execute('''SELECT * FROM orders \
    ...
    # Expire order_matches for GASP with no GASP.
    cursor.execute('''SELECT * FROM order_matches WHERE (status = ? and match_expire_index < ?)''', ('pending', block_index))
