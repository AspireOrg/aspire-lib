...
from aspirelib.test.fixtures.vectors import UNITTEST_VECTOR
from aspirelib.test.fixtures.params import DEFAULT_PARAMS
from aspirelib.test.fixtures.scenarios import INTEGRATION_SCENARIOS

from aspirelib.lib import config, util, database, api, script, arc4
...
def init_mock_functions(request, monkeypatch, mock_utxos, rawtransactions_db):
    ...
    monkeypatch.setattr('aspirelib.lib.transaction.arc4.init_arc4', init_arc4)
    monkeypatch.setattr('aspirelib.lib.backend.get_unspent_txouts', get_unspent_txouts)
    monkeypatch.setattr('aspirelib.lib.log.isodt', isodt)
    monkeypatch.setattr('aspirelib.lib.log.curr_time', curr_time)
    monkeypatch.setattr('aspirelib.lib.util.date_passed', date_passed)
    monkeypatch.setattr('aspirelib.lib.api.init_api_access_log', init_api_access_log)
    if hasattr(config, 'PREFIX'):
        monkeypatch.setattr('aspirelib.lib.config.PREFIX', b'TESTXXXX')
    monkeypatch.setattr('aspirelib.lib.backend.getrawtransaction', mocked_getrawtransaction)
    monkeypatch.setattr('aspirelib.lib.backend.getrawtransaction_batch', mocked_getrawtransaction_batch)
    monkeypatch.setattr('aspirelib.lib.backend.search_raw_transactions', mocked_search_raw_transactions)
    monkeypatch.setattr('aspirelib.lib.backend.pubkeyhash_to_pubkey', pubkeyhash_to_pubkey)
    monkeypatch.setattr('aspirelib.lib.backend.multisig_pubkeyhashes_to_pubkeys', multisig_pubkeyhashes_to_pubkeys)
