...
from aspirelib.lib.messages.scriptlib.processblock import ContractError
from aspirelib.lib.api import APIError
from aspirelib.lib.util import (DebitError, CreditError, QuantityError, RPCError)
...
UNITTEST_VECTOR = {
    ...
    'broadcast': {
        'validate': [{
            ...
            'mock_protocol_changes': {'enhanced_sends': True, 'options_require_memo': True},
            'in': (ADDR[5], 1588000000, 1, DP['fee_multiplier'], 'OPTIONS ASP', DP['default_block_index']),
    ...
    'send': {
        'validate': [{
            'in': (ADDR[0], ADDR[1], 'ASP', DP['quantity'], 1),
            'out': ([])
        }, {
            'in': (ADDR[0], P2SH_ADDR[0], 'ASP', DP['quantity'], 1),
            'out': ([])
        }, {
            'in': (P2SH_ADDR[0], ADDR[1], 'ASP', DP['quantity'], 1),
            'out': ([])
        }, {
            ...
            'in': (ADDR[0], ADDR[1], 'ASP', -1 * DP['quantity'], 1),
            'out': (['negative quantity'])
        }, {
            'in': (ADDR[0], MULTISIGADDR[0], 'ASP', DP['quantity'], 1),
            'out': ([])
        }, {
            ...
            'in': (ADDR[0], ADDR[6], 'ASP', DP['quantity'], 1),
            'out': (['destination requires memo'])
        }],
        'compose': [{
            'in': (ADDR[0], ADDR[1], 'ASP', DP['small']),
            ...
            'in': (P2SH_ADDR[0], ADDR[1], 'ASP', DP['small']),
            ...
            'in': (ADDR[0], P2SH_ADDR[0], 'ASP', DP['small']),
            ...
            'in': (ADDR[0], ADDR[1], 'ASP', DP['quantity'] * 10000000),
            'error': (exceptions.ComposeError, 'insufficient funds')
        }, {
            'in': (ADDR[0], ADDR[1], 'ASP', DP['quantity'] / 3),
            'error': (exceptions.ComposeError, 'quantity must be an int (in satoshi)')
        }, {
            'in': (ADDR[0], MULTISIGADDR[0], 'ASP', DP['quantity']),
            'out': ('mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc',
            ...
            'in': (ADDR[0], ADDR[1], 'GASP', DP['quantity']),
            'out': ('mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc',
                    [('mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', 100000000)],
                    None)
        }, {
            'in': (ADDR[0], P2SH_ADDR[0], 'GASP', DP['quantity']),
            'out': ('mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc',
            ...
            'in': (ADDR[0], ADDR[6], 'ASP', 100000000),
            ...
            'in': ({'source': ADDR[0], 'destination': ADDR[6], 'asset': 'ASP', 'quantity': 100000000, 'memo': '12345', 'use_enhanced_send': True}),
    ...
    'issuance': {
        'validate': [{
            ...
            'in': (ADDR[0], None, 'GASP', 1000, True, False, None, None, '', None, None, DP['default_block_index']),
            'out': (0, 0.0, ['cannot issue GASP or ASP'], 50000000, '', True, False, None)
        }, {
            'in': (ADDR[0], None, 'ASP', 1000, True, False, None, None, '', None, None, DP['default_block_index']),
            'out': (0, 0.0, ['cannot issue GASP or ASP'], 50000000, '', True, False, None)
        }, {
    ...
    'dividend': {
        'validate': [{
            'in': (ADDR[0], DP['quantity'] * 1000, 'DIVISIBLE', 'ASP', DP['default_block_index']),
            'out': (1200000000000,
                    [
            ...
            'in': (ADDR[0], DP['quantity'], 'GASP', 'ASP', DP['default_block_index']),
            'out': (None, None, ['cannot pay dividends to holders of BTC', 'no such asset, BTC.'], 0)
        }, {
            'comment': 'cannot pay dividends to holders of ASP',
            'in': (ADDR[0], DP['quantity'], 'ASP', 'ASP', DP['default_block_index']),
            'out': (None, None, ['cannot pay dividends to holders of ASP', 'no such asset, ASP.'], 0)
        }, {
            'comment': 'no such asset, NOASSET',
            'in': (ADDR[0], DP['quantity'], 'NOASSET', 'ASP', DP['default_block_index']),
            'out': (None, None, ['no such asset, NOASSET.'], 0)
        }, {
            'comment': 'non‐positive quantity per unit',
            'in': (ADDR[0], 0, 'DIVISIBLE', 'ASP', DP['default_block_index']),
            'out': (0,
                    [
            ...
            'in': (ADDR[1], DP['quantity'], 'DIVISIBLE', 'ASP', DP['default_block_index']),
            'out': (99900000000,
                    [
                    ...
                    ['only issuer can pay dividends', 'insufficient funds (ASP)'],
                    0)
        }, {
            'in': (ADDR[0], DP['quantity'], 'DIVISIBLE', 'NOASSET', DP['default_block_index']),
            'out': (None, None, ['no such dividend asset, NOASSET.'], 0)
        }, {
            'in': (ADDR[0], 8359090909, 'DIVISIBLE', 'ASP', DP['default_block_index']),
            'out': (100309090908,
                    [
        ...
        'compose': [{
            'in': (ADDR[0], DP['quantity'], 'DIVISIBLE', 'ASP'),
    ...
    'order': {
        'validate': [{
            'in': (ADDR[0], 'DIVISIBLE', DP['quantity'], 'ASP', DP['quantity'], 2000, 0, DP['default_block_index']),
            'out': ([])
        }, {
            'in': (P2SH_ADDR[0], 'DIVISIBLE', DP['quantity'], 'ASP', DP['quantity'], 2000, 0, DP['default_block_index']),
            'out': ([])
        }, {
            'in': (ADDR[0], 'DIVISIBLE', DP['quantity'], 'ASP', DP['quantity'], 2000, 0.5, DP['default_block_index']),
            'out': (['fee_required must be in satoshis'])
        }, {
            'in': (ADDR[0], 'GASP', DP['quantity'], 'GASP', DP['quantity'], 2000, 0, DP['default_block_index']),
            'out': (['cannot trade BTC for itself'])
        }, {
            'in': (ADDR[0], 'DIVISIBLE', DP['quantity'] / 3, 'ASP', DP['quantity'], 2000, 0, DP['default_block_index']),
            'out': (['give_quantity must be in satoshis'])
        }, {
            'in': (ADDR[0], 'DIVISIBLE', DP['quantity'], 'ASP', DP['quantity'] / 3, 2000, 0, DP['default_block_index']),
            'out': (['get_quantity must be in satoshis'])
        }, {
            'in': (ADDR[0], 'DIVISIBLE', DP['quantity'], 'ASP', DP['quantity'], 1.5, 0, DP['default_block_index']),
            'out': (['expiration must be expressed as an integer block delta'])
        }, {
            'in': (ADDR[0], 'DIVISIBLE', -DP['quantity'], 'ASP', -DP['quantity'], -2000, -10000, DP['default_block_index']),
            'out': (['non‐positive give quantity', 'non‐positive get quantity', 'negative fee_required', 'negative expiration'])
        }, {
            'in': (ADDR[0], 'DIVISIBLE', 0, 'ASP', DP['quantity'], 2000, 0, DP['default_block_index']),
            ...
            'in': (ADDR[0], 'DIVISIBLE', 2**63 + 10, 'ASP', DP['quantity'], 4 * 2016 + 10, 0, DP['default_block_index']),
            'out': (['integer overflow', 'expiration overflow'])
        }],
        'compose': [{
            'in': (ADDR[0], 'GASP', DP['small'], 'ASP', DP['small'] * 2, DP['expiration'], 0),
            ...
            'in': (P2SH_ADDR[0], 'GASP', DP['small'], 'ASP', DP['small'] * 2, DP['expiration'], 0),
            'out': (P2SH_ADDR[0], [], b'\x00\x00\x00\n\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\xfa\xf0\x80\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x05\xf5\xe1\x00\x00\n\x00\x00\x00\x00\x00\x00\x00\x00')
        }, {
            'in': (ADDR[0], 'ASP', round(DP['small'] * 2.1), 'GASP', DP['small'], DP['expiration'], DP['fee_required']),
            ...
            'in': (MULTISIGADDR[0], 'GASP', DP['small'], 'ASP', DP['small'] * 2, DP['expiration'], 0),
            'out': ('1_mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc_mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns_2', [], b'\x00\x00\x00\n\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\xfa\xf0\x80\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x05\xf5\xe1\x00\x00\n\x00\x00\x00\x00\x00\x00\x00\x00')
        }, {
            'in': (MULTISIGADDR[0], 'ASP', round(DP['small'] * 2.1), 'GASP', DP['small'], DP['expiration'], DP['fee_required']),
            'out': ('1_mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc_mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns_2', [], b'\x00\x00\x00\n\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x06B,@\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\xfa\xf0\x80\x00\n\x00\x00\x00\x00\x00\r\xbb\xa0')
        }, {
            'in': (ADDR[0], 'MAXI', 2**63 - 1, 'ASP', DP['quantity'], DP['expiration'], DP['fee_required']),
            ...
            'in': (ADDR[0], 'MAXI', 2**63 - 1, 'ASP', DP['quantity'], DP['expiration'], 2 ** 63),
            'error': (exceptions.ComposeError, "['integer overflow']")
        }, {
            'in': (ADDR[0], 'MAXI', 2**63, 'ASP', DP['quantity'], DP['expiration'], DP['fee_required']),
            'error': (exceptions.ComposeError, 'insufficient funds')
        }, {
            'comment': 'give subasset',
            'in': (ADDR[0], 'PARENT.already.issued', 100000000, 'ASP', DP['small'], DP['expiration'], DP['fee_required']),
            'out': ('mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc', [],
            ...
            'in': (ADDR[0], 'ASP', DP['small'], 'PARENT.already.issued', 100000000, DP['expiration'], DP['fee_required']),
            'out': ('mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc', [],
    ...
    'api': {
        'get_rows': [{
            ...
            'out': [{'tx_index': 497, 'tx_hash': '478048dcdaca3e8010fc50a75511f6ed8fc1a770a4fab5c339a1c15c3633971f', 'block_index': 310496, 'source': 'mnfAHmddVibnZNSkh8DvKaQoiEfNsxjXzH', 'destination': 'mqPCfvqTfYctXMUfmniXeG2nyaN8w6tPmj', 'asset': 'ASP', 'quantity': 92945878046, 'status': 'valid', 'memo': None, 'memo_hex': None}]
        }, {
            'comment': 'with memo',
            'in': ('sends', [{'field':'block_index','op':'=','value':'310481'}], 'AND', None, None, None, None, None, 1000, 0, True),
            'out': [{'tx_index': 482, 'tx_hash': 'e864d7881d5c6a57b650dd507096275647eb32c05e3a27815869133a79a9db6f', 'block_index': 310481, 'source': 'mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc', 'destination': 'mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', 'asset': 'ASP', 'quantity': 100000000, 'status': 'valid', 'memo': 'hello', 'memo_hex': '68656c6c6f'}]
        }, {
            ...
            'out': [{'tx_index': 483, 'tx_hash': 'b9ad301585a4f779a15adb7c357215918a22a0c32d78f1fc622122dce2ba46c2', 'block_index': 310482, 'source': 'mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', 'destination': 'mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc', 'asset': 'ASP', 'quantity': 100000000, 'status': 'valid', 'memo': '', 'memo_hex': 'fade0001'}]
        }],
    },
    'script': {
        'validate': [{
    ...
    'util': {
        'api': [{
            ...
            'in': ('create_send', {'source': ADDR[0], 'destination': ADDR[1], 'asset': 'ASP', 'quantity': DP['small'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_send', {'source': P2SH_ADDR[0], 'destination': ADDR[1], 'asset': 'ASP', 'quantity': DP['small'], 'encoding': 'multisig', 'dust_return_pubkey': False}),
            'out': ''
        }, {
            ...
            'in': ('create_order', {'source': ADDR[0], 'give_asset': 'GASP', 'give_quantity': DP['small'], 'get_asset': 'ASP', 'get_quantity': DP['small'] * 2, 'expiration': DP['expiration'], 'fee_required': 0, 'fee_provided': DP['fee_provided'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_order', {'source': ADDR[0], 'give_asset': 'ASP', 'give_quantity': round(DP['small'] * 2.1), 'get_asset': 'GASP', 'get_quantity': DP['small'], 'expiration': DP['expiration'], 'fee_required': DP['fee_required'], 'encoding': 'multisig'}),
            ...
            'in': ('create_send', {'source': ADDR[0], 'destination': MULTISIGADDR[0], 'asset': 'ASP', 'quantity': DP['quantity'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_send', {'source': MULTISIGADDR[0], 'destination': ADDR[0], 'asset': 'ASP', 'quantity': DP['quantity'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_send', {'source': MULTISIGADDR[0], 'destination': MULTISIGADDR[1], 'asset': 'ASP', 'quantity': DP['quantity'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            ...
            'comment': '1',
            'in': ('create_order', {'source': MULTISIGADDR[0], 'give_asset': 'GASP', 'give_quantity': DP['small'], 'get_asset': 'ASP', 'get_quantity': DP['small'] * 2, 'expiration': DP['expiration'], 'fee_required': 0, 'fee_provided': DP['fee_provided'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_order', {'source': MULTISIGADDR[0], 'give_asset': 'ASP', 'give_quantity': round(DP['small'] * 2.1), 'get_asset': 'GASP', 'get_quantity': DP['small'], 'expiration': DP['expiration'], 'fee_required': DP['fee_required'], 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_dividend', {'source': ADDR[0], 'quantity_per_unit': DP['quantity'], 'asset': 'DIVISIBLE', 'dividend_asset': 'ASP', 'encoding': 'multisig'}),
            'out': ''
        }, {
            'in': ('create_dividend', {'source': ADDR[0], 'quantity_per_unit': 1, 'asset': 'NODIVISIBLE', 'dividend_asset': 'ASP', 'encoding': 'multisig'}),
            'out': ''
        ...
        # CIP 9 enhanced_send tests
            ...
            'in': ('create_send', {'source': ADDR[0], 'destination': ADDR[1], 'asset': 'ASP', 'quantity': DP['small']}),
            ...
            'in': ('create_send', {'use_enhanced_send': False, 'source': ADDR[0], 'destination': ADDR[1], 'asset': 'ASP', 'quantity': DP['small']}),
            'out': '01000000'+'01'+'c1d8c075936c3495f6d653c50f73d987f75448d97a750249b1eb83bee71b24ae'+'00000000'+'19'+'76a9144838d8b3588c4c7ba7c1d06f866e9b3739c6303788ac'+'ffffffff'+'03'+'3615000000000000'+'19'+'76a9148d6ae8a3b381663118b4e1eff4cfc7d0954dd6ec88ac'+'0000000000000000'+'1e'+'6a1c2a504df746f83442653dd7ada4dc727a030865749e9fba5aec80c39a'+'1b2bea0b00000000'+'19'+'76a9144838d8b3588c4c7ba7c1d06f866e9b3739c6303788ac'+'00000000'
        }, {
            ...
            'mock_protocol_changes': {'enhanced_sends': True},
            'in': ('create_send', {'memo': 'hello', 'source': ADDR[0], 'destination': ADDR[1], 'asset': 'ASP', 'quantity': DP['small']}),
            ...
            'mock_protocol_changes': {'enhanced_sends': True},
            'in': ('create_send', {'memo': '0102030405', 'memo_is_hex': True, 'source': ADDR[0], 'destination': ADDR[1], 'asset': 'ASP', 'quantity': DP['small']}),
            ...
            'mock_protocol_changes': {'enhanced_sends': True, 'options_require_memo': True},
            'in': ('create_send', {'source': ADDR[0], 'destination': ADDR[6], 'asset': 'ASP', 'quantity': DP['small']}),
            ...
            'mock_protocol_changes': {'enhanced_sends': True, 'options_require_memo': True},
            'in': ('create_send', {'memo': '0102030405', 'memo_is_hex': True, 'source': ADDR[0], 'destination': ADDR[6], 'asset': 'ASP', 'quantity': DP['small']}),
        ...
        'generate_asset_id': [{
            'in': ('GASP', DP['default_block_index']),
            'out': 0
        }, {
            'in': ('ASP', DP['default_block_index']),
            'out': 1
        }, {
            'in': ('BCD', 308000),
            'error': (exceptions.AssetNameError, 'too short')
        ...
        'debit': [{
            ...
            'in': (ADDR[0], 'GASP', 1.1 * DP['quantity']),
        ...
        {
            'in': ('ASP.child1',),
            'error': (exceptions.AssetNameError, "parent asset cannot be ASP")
        },
        {
            'in': ('PARENT.',),
    ...
    'versions.enhanced_send': {
        ...
        {
            'in': (ADDR[0], ADDR[1], 'ASP', DP['quantity'], None, 1),
            'out': ([])
        }, {
            'in': (ADDR[0], P2SH_ADDR[0], 'ASP', DP['quantity'], None, 1),
            'out': ([])
        }, {
            'in': (P2SH_ADDR[0], ADDR[1], 'ASP', DP['quantity'], None, 1),
            'out': ([])
        }, {
            'in': (ADDR[0], ADDR[1], 'GASP', DP['quantity'], None, 1),
            'out': (['cannot send {}'.format(config.BTC)])
        }, {
            'in': (ADDR[0], ADDR[1], 'ASP', DP['quantity'] / 3, None, 1),
            'out': (['quantity must be in satoshis'])
        }, {
            'in': (ADDR[0], ADDR[1], 'ASP', -1 * DP['quantity'], None, 1),
            'out': (['negative quantity'])
        }, {
            'in': (ADDR[0], MULTISIGADDR[0], 'ASP', DP['quantity'], None, 1),
            'out': ([])
        }, {
            ...
            'in': (ADDR[0], ADDR[6], 'ASP', DP['quantity'], None, 1),
            'out': (['destination requires memo'])
        }, {
        ...
        # ----- tests copied from regular send -----
        {
            'in': (ADDR[0], ADDR[1], 'ASP', DP['quantity'] * 10000000, None, False),
            'error': (exceptions.ComposeError, 'insufficient funds')
        }, {
            'in': (ADDR[0], ADDR[1], 'ASP', DP['quantity'] / 3, None, False),
            'error': (exceptions.ComposeError, 'quantity must be an int (in satoshi)')
        }, {
            'in': (ADDR[0], ADDR[1], 'MAXI', 2**63 + 1, None, False),
            'error': (exceptions.ComposeError, 'insufficient funds')
        }, {
            'in': (ADDR[0], ADDR[1], 'GASP', DP['quantity'], None, False),
            'out': ('mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc',
                    [('mtQheFaSfWELRB2MyMBaiWjdDm6ux9Ezns', 100000000)],
                    None)
        }, {
            'in': (ADDR[0], P2SH_ADDR[0], 'GASP', DP['quantity'], None, False),
            'out': ('mn6q3dS2EnDUx3bmyWc6D4szJNVGtaR7zc',
                    [('2MyJHMUenMWonC35Yi6PHC7i2tkS7PuomCy', 100000000)],
                    None)
        }, {
        ...
        # ----- tests specific to enhanced send -----
        {
            ...
            'in': (ADDR[0], ADDR[6], 'ASP', DP['small'], 'deadbeef', True),
