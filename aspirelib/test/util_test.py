...
from aspirelib.lib import (config, util, blocks, check, backend, database, transaction)

from aspirelib.test.fixtures.params import DEFAULT_PARAMS as DP
from aspirelib.test.fixtures.scenarios import UNITTEST_FIXTURE, INTEGRATION_SCENARIOS, standard_scenarios_params
...
def init_database(sqlfile, dbfile, options=None):
    ...
    server.initialise(
        ...
        testnet=True,
        verbose=True,
        console_logfilter=os.environ.get('ASPIRE_LOGGING', None),
        **kwargs)
...
def initialise_rawtransactions_db(db):
    ...
    if pytest.config.option.savescenarios:
        server.initialise(database_file=':memory:', testnet=True, **ASPIRED_OPTIONS)
        cursor = db.cursor()
    ...
    for tx in scenario:
        if tx[0] != 'create_next_block':
            ...
            with MockProtocolChangesContext(**(mock_protocol_changes or {})):
                module = sys.modules['aspirelib.lib.messages.{}'.format(tx[0])]
...
def check_outputs(tx_name, method, inputs, outputs, error, records, comment, mock_protocol_changes, server_db):
    ...
    try:
        tested_module = sys.modules['aspirelib.lib.{}'.format(tx_name)]
    except KeyError:    # TODO: hack
        tested_module = sys.modules['aspirelib.lib.messages.{}'.format(tx_name)]
    ...
    with MockProtocolChangesContext(**(mock_protocol_changes or {})):
        ...
        if error is not None:
            ...
            if pytest.config.option.gentxhex and method == 'compose':
                ...
                if tx_name == 'order' and inputs[1]=='GASP':
                    print('give gasp')
...
class MockProtocolChangesContext(object):
    def __init__(self, **kwargs):
        from aspirelib.test.conftest import MOCK_PROTOCOL_CHANGES
