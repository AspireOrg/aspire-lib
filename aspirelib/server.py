...
from aspirelib.lib import api, config, util, exceptions, blocks, check, backend, database, transaction, script
...
def initialise_config(database_file=None, log_file=None, api_log_file=None,
    ...
    # Backend Core RPC port (AspireGas Core)
    if backend_port:
    ...
    # Backend Core RPC user (AspireGas Core)
    if backend_user:
    ...
    logger.info('Running v{} of aspire-lib.'.format(config.VERSION_STRING))
...
def kickstart(db, gaspd_dir):
    blocks.kickstart(db, gaspd_dir=gaspd_dir)
